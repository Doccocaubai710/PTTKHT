# Draft — Sequence Diagram cho UC003 (Đặt sân)

## Luồng chính (thành công)

```
1.  Khách hàng -> BookingPage.book_slot(slot_id)
2.  BookingPage -> BookingService.create_booking(field_id, time_slot_id, booking_date, customer_id, created_by_id)
...
11. Database -> Database: kiểm tra partial UNIQUE INDEX (field_id, booking_date, time_slot_id) WHERE status IN (PENDING, CONFIRMED, COMPLETED)
...
15. BookingService --> BookingPage: booking (status=PENDING, hold_expires_at=now+10p)
16. BookingPage --> Khách hàng: thông báo "Đặt sân thành công, vui lòng thanh toán cọc trong 10 phút" + điều hướng "/my-bookings"
```

## Luồng ngoại lệ A1 — Trùng lịch (2 khách đặt cùng lúc)

```
1.  Khách hàng A -> BookingPage.book_slot(slot_id)
2.  BookingPage -> BookingService.create_booking(...)
...
4'. Khách hàng B -> BookingPage.book_slot(slot_id)  [cùng field_id, cùng ngày, cùng time_slot_id]
5'. BookingPage -> BookingService.create_booking(...)
...
10. BookingService (A) --> BookingPage (A): booking thành công
...
16. BookingService --> BookingPage (B): BookingError
17. BookingPage --> Khách hàng B: hiển thị thông báo lỗi nghiệp vụ (ui.notify, type=negative)
```
