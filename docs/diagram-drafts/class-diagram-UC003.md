# Draft — Class Diagram cho UC003 (Đặt sân)

## Danh sách lớp tham gia

### 1. `BookingPage` (presentation — `app/pages/customer_pages.py::field_detail_page`)
...
- Giá trị: `PENDING, CONFIRMED, COMPLETED, CANCELLED, EXPIRED`

## Quan hệ giữa các lớp

| Từ lớp | Đến lớp | Loại quan hệ | Bội số |
|---|---|---|---|
| `BookingPage` | `BookingService` | Dependency (gọi phương thức, không giữ tham chiếu lâu dài) | 1..1 |
...
- `Booking` có ràng buộc **unique index** trên tổ hợp `(field_id, booking_date,
  time_slot_id)` (chỉ tính khi `status` thuộc {PENDING, CONFIRMED, COMPLETED}) —
  nên ghi chú (constraint note) đính kèm lớp `Booking` trên class diagram, vì đây
  là ràng buộc dữ liệu quan trọng nhất của UC003, không thể hiện được qua bội số
  association thông thường.
